...
const Item = ({ item, todos, setTodos }) => {
  ...
  // console.log("in list cheching item", item);
  const completeTodo = () => {
    setTodos((prevTodos) =>
      prevTodos.map((todo) =>
        todo.id === item.id
  ...
  const handleInputSubmit = async (event) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost/4000/api/todo/${item.id}`,
        item
      );
      console.log("Todo updated successfully:", response.data.todo);
      // Handle the updated todo data as needed
    } catch (error) {
      if (error.response) {
  ...
  const handleDelete = async () => {
    // Update the todos state and localStorage after deleting todo
    setTodos((prevTodos) => prevTodos.filter((todo) => todo.id !== item.id));
    const updatedTodos = JSON.stringify(
      todos.filter((todo) => todo.id !== item.id)
    );
    console.log("deleteing it", item.id);
    try {
      await axios.delete(`http://localhost:4000/api/todo/${item.id}`); // Include todo ID in the URL
      console.log("Todo deleted successfully");
    } catch (errors) {
      console.log("Error deleting todo:", errors);
    }
    localStorage.setItem("todos", updatedTodos);
  };
